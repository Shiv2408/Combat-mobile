...
export default function HomeScreen() {
  ...
  const getDisplayName = () => {
    if (isFighter && userData.roleData?.fightName) {
      return userData.roleData.fightName;
    }
    if (isGym && userData.roleData?.gymName) {
          ...
          <View style={styles.personalStatsCard}>
            ...
            {isPromoter && (
              <>
                ...
                <View style={styles.personalStat}>
                  <Text style={[styles.personalStatNumber, { color: '#2196F3' }]}>{userStats.completedEvents || 0}</Text>
                  <Text style={styles.personalStatLabel}>Completed</Text>
                </View>
              </>
            )}
            {isGym && (
