...
import FighterCard from '../search/FighterCard';
import PromoterCard from '../search/PromoterCard';
import GymCard from '../search/GymCard';
...
export default function CommunityScreen() {
  ...
  const allFighters = useQuery(api.fighters.getAllFighters, {});
  const allPromoters = useQuery(api.promoters.getAllPromoters, {});
  const allGyms = useQuery(api.gyms.getAllGyms, {});
  ...
  const getFilteredData = () => {
    ...
    switch (activeTab) {
      case 'fighters':
        data = Array.isArray(allFighters)
          ? allFighters
          : allFighters?.page || [];
        break;
      case 'promoters':
  ...
  const getFightersCount = () => {
    if (Array.isArray(allFighters)) return allFighters.length;
    if (allFighters && Array.isArray(allFighters.page)) return allFighters.page.length;
    return 0;
  };

  const tabs = [
    { key: 'fighters', label: 'Fighters', icon: Shield, count: getFightersCount(), gradient: ['#4CAF50', '#45a049'] as const },
        ...
        <View style={styles.headerContent}>
          ...
          <View style={styles.headerStats}>
            <Text style={styles.totalMembers}>
              {getFightersCount() + (allPromoters?.length || 0) + (allGyms?.length || 0)}
            </Text>
